...
--                 installTargets += env.Install(os.path.join(distDir, 'data/microfacet'), '#data/microfacet/' + file)
    on_build(function (target)
        os.trycp("data/schema", path.join(target:targetdir(), "data/"))
        os.trycp("data/ior", path.join(target:targetdir(), "data/"))
        os.trycp("data/microfacet", path.join(target:targetdir(), "data/"))
    end)