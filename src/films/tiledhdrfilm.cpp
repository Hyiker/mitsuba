 ...
 *         \code{xyza}, \code{spectrum}, and \code{spectrumAlpha}. In the latter
 * two cases, the number of written channels depends on the value assigned to
 *         \code{SPECTRUM\_SAMPLES} during compilation (see
 * Section~\ref{sec:compiling} for details) \default{\code{rgb}}
 *     }
 ...
 *     be used by the film. \default{\code{gaussian}, a windowed Gaussian
 * filter}}
 * }
 *
 ...
 *
 * Based on the provided parameter values, the film will either write a
 * luminance, luminance/alpha, RGB(A), XYZ(A) tristimulus, or
 * spectrum/spectrum-alpha-based bitmap having a \code{float16}, \code{float32},
 * or \code{uint32}-based internal representation. The default is RGB and
 * \code{float16}. Note that the spectral output options only make sense when
 * using a custom compiled Mitsuba distribution that has spectral rendering
...
class TiledHDRFilm : public Film {
   public:
    TiledHDRFilm(const Properties &props)
        : Film(props), m_output(NULL), m_frameBuffer(NULL) {
        std::vector<std::string> pixelFormats = tokenize(
            boost::to_lower_copy(props.getString("pixelFormat", "rgb")), " ,");
        std::vector<std::string> channelNames =
            tokenize(props.getString("channelNames", ""), ", ");
        std::string componentFormat =
            boost::to_lower_copy(props.getString("componentFormat", "float16"));

        if (pixelFormats.empty())
            Log(EError, "At least one pixel format must be specified!");

        if ((pixelFormats.size() != 1 &&
             channelNames.size() != pixelFormats.size()) ||
            (pixelFormats.size() == 1 && channelNames.size() > 1))
            Log(EError,
                "Number of channel names must match the number of specified "
                "pixel formats!");

        for (size_t i = 0; i < pixelFormats.size(); ++i) {
            std::string pixelFormat = pixelFormats[i];
            std::string name = i < channelNames.size()
                                   ? (channelNames[i] + std::string("."))
                                   : "";
            ...
            if (pixelFormat == "luminance") {
                ...
                for (int i = 0; i < SPECTRUM_SAMPLES; ++i) {
                    std::pair<Float, Float> coverage =
                        Spectrum::getBinCoverage(i);
                    m_channelNames.push_back(
                        name + formatString("%.2f-%.2fnm", coverage.first,
                                            coverage.second));
                }
                ...
                Log(EError,
                    "The \"pixelFormat\" parameter must either be equal to "
                    "\"luminance\", \"luminanceAlpha\", \"rgb\", \"rgba\", "
                    "\"xyz\", \"xyza\", "
        ...
        for (size_t i = 0; i < m_pixelFormats.size(); ++i) {
            if (SPECTRUM_SAMPLES == 3 &&
                (m_pixelFormats[i] == Bitmap::ESpectrum ||
                 m_pixelFormats[i] == Bitmap::ESpectrumAlpha))
                Log(EError,
                    "You requested to render a spectral image, but Mitsuba is "
                    "currently "
                    "configured for a RGB flow (i.e. SPECTRUM_SAMPLES = 3). "
                    "You will need to recompile "
                    "it with a different configuration. Please see the "
        ...
        if (componentFormat == "float16") {
            ...
            Log(EError,
                "The \"componentFormat\" parameter must either be "
                "equal to \"float16\", \"float32\", or \"uint32\"!");
        }

        if (m_highQualityEdges)
            Log(EError,
                "The 'highQualityEdges' parameter is incompatible with the "
    ...
    virtual ~TiledHDRFilm() { develop(NULL, 0); }
    ...
    void setDestinationFile(const fs::path &destFile, uint32_t blockSize) {
        if (m_output) develop(NULL, 0);

        fs::path filename = destFile;
        std::string extension =
            boost::to_lower_copy(filename.extension().string());
        if (extension != ".exr") filename.replace_extension(".exr");

        Log(EInfo, "Commencing creation of a tiled EXR image at \"%s\" ..",
            filename.string().c_str());

        Imf::Header header(m_size.x, m_size.y);
        header.setTileDescription(
            Imf::TileDescription(blockSize, blockSize, Imf::ONE_LEVEL));
        header.insert("generated-by",
                      Imf::StringAttribute("Mitsuba version " MTS_VERSION));
        ...
        if (m_pixelFormats.size() == 1) {
            ...
            if (pixelFormat == Bitmap::EXYZ || pixelFormat == Bitmap::EXYZA) {
                Imf::addChromaticities(
                    header, Imf::Chromaticities(
                                Imath::V2f(1.0f, 0.0f), Imath::V2f(0.0f, 1.0f),
                                Imath::V2f(0.0f, 0.0f),
                                Imath::V2f(1.0f / 3.0f, 1.0f / 3.0f)));
            } else if (pixelFormat == Bitmap::ERGB ||
        ...
        if (m_componentFormat == Bitmap::EFloat16) {
            ...
            Log(EError,
                "Invalid component type (must be "
    ...
    void put(const ImageBlock *block) {
        ...
        for (int yo = -1; yo <= 1; ++yo)
            for (int xo = -1; xo <= 1; ++xo) potentiallyWrite(x + xo, y + yo);
    }

    void setBitmap(const Bitmap *bitmap, Float multiplier) {
        Log(EError,
            "setBitmap(): Global image updates are permitted by this film, "
            "which operates strictly on tiles! Please either switch to a "
            "compatible "
    ...
    void addBitmap(const Bitmap *bitmap, Float multiplier) {
        Log(EError,
            "addBitmap(): Global image updates are permitted by this film, "
            "which operates strictly on tiles! Please either switch to a "
            "compatible "
            "rendering technique or use a non-tiled film. (e.g. 'hdrfilm')");
    }
    ...
    void potentiallyWrite(int x, int y) {
        if (x < 0 || y < 0 || x >= m_blocksH || y >= m_blocksV) return;

        uint32_t idx = (uint32_t)x + (uint32_t)y * m_blocksH;
        std::map<uint32_t, ImageBlock *>::iterator it = m_origBlocks.find(idx);
        if (it == m_origBlocks.end()) return;

        ImageBlock *origBlock = it->second;
        if (origBlock == NULL) return;
        ...
        for (int yo = -1; yo <= 1; ++yo) {
            for (int xo = -1; xo <= 1; ++xo) {
                ...
                if (!origBlock2 || !mergedBlock2) continue;
        ...
        for (int i = 0; i < m_blockSize; ++i) {
            if (m_pixelFormats.size() == 1)
                cvt->convert(source->getPixelFormat(), 1.0f, sourceData,
                             m_tile->getPixelFormat(), m_tile->getGamma(),
                             targetData, m_tile->getWidth());
            else
                Bitmap::convertMultiSpectrumAlphaWeight(
                    source, sourceData, m_tile, targetData, m_pixelFormats,
                    m_componentFormat, m_tile->getWidth());
    ...
    void develop(const Scene *scene, Float renderTime) {
        if (m_output) {
            Log(EInfo,
                "Closing EXR file (%u tiles in total, peak memory usage: %u "
                "tiles)..",
            ...
            for (std::map<uint32_t, ImageBlock *>::iterator it =
                     m_origBlocks.begin();
