...
#define XMLLog(level, fmt, ...)                                                \
    Thread::getThread()->getLogger()->log(                                     \
        level, NULL, __FILE__, __LINE__,                                       \
        "In file \"%s\" (near line %i): " fmt,                                 \
        m_locator ? transcode(m_locator->getSystemId()).c_str() : "<unknown>", \
        m_locator ? m_locator->getLineNumber() : -1, ##__VA_ARGS__)
...
SceneHandler::SceneHandler(const ParameterMap &params,
                           NamedObjectMap *namedObjects, bool isIncludedFile)
    : m_params(params),
      m_namedObjects(namedObjects),
...
SceneHandler::~SceneHandler() {
    delete m_transcoder;
    clear();
    if (!m_isIncludedFile) delete m_namedObjects;
}
...
std::string SceneHandler::transcode(const XMLCh *input) const {
    ...
    while (totalCharsConsumed < charsToBeConsumed) {
        ...
        XMLSize_t charsProduced = m_transcoder->transcodeTo(
            input,
...
void SceneHandler::startDocument() { clear(); }
...
void SceneHandler::endDocument() {
    ...
    for (CleanupSet::iterator it = cleanup.begin(); it != cleanup.end(); ++it)
        (*it)();
    cleanup.clear();
}

void SceneHandler::characters(const XMLCh *const name, const XMLSize_t length) {
    std::string value = trim(transcode(name));
    if (value != "")
        XMLLog(EWarn, "Unexpected character data: %s", value.c_str());
}

Float SceneHandler::parseFloat(const std::string &name, const std::string &str,
                               Float defVal) const {
    char *end_ptr = NULL;
    if (str.empty()) {
        if (defVal == -1)
            XMLLog(EError, "Missing floating point value (in <%s>)",
                   name.c_str());
        return defVal;
    }
    ...
    if (*end_ptr != '\0')
        XMLLog(EError, "Invalid floating point value specified (in <%s>)",
               name.c_str());
    return result;
}
...
void SceneHandler::startElement(const XMLCh *const xmlName,
    ...
    for (size_t i = 0; i < xmlAttributes.getLength(); i++) {
        ...
        if (attrValue.length() > 0 && attrValue.find('$') != attrValue.npos) {
            for (ParameterMap::const_reverse_iterator it = m_params.rbegin();
                 it != m_params.rend(); ++it) {
                std::string::size_type pos = 0;
                std::string searchString = "$" + it->first;
                while ((pos = attrValue.find(searchString, pos)) !=
                       std::string::npos) {
            ...
            if (attrValue.find('$') != attrValue.npos &&
                attrValue.find('[') == attrValue.npos)
                XMLLog(EError,
                       "The scene referenced an undefined parameter: \"%s\"",
                       attrValue.c_str());
        }
        ...
        case EScene: {
            ...
            if (versionString == "")
                throw VersionException(
                    formatString(
                        "The requested scene cannot be loaded, since it "
                        "is missing version information! Since Mitsuba 0.3.0, "
                        "it is "
                        "mandatory that scene XML files specify the version of "
                        "Mitsuba "
                        "that was used at the time of their creation.\nThis "
                        "makes it clear "
                        "how to interpret them in the presence of a changing "
                        "file format. "
                        "The version should be specified within the 'scene' "
                        "tag, "
                        "e.g.\n\t<scene version=\"" MTS_VERSION "\">\n"
                        "Please update your scene file with the right version "
                        "number and try reloading it."),
                    Version());
            ...
            if (!fileVersion.isCompatible(currentVersion)) {
                if (fileVersion < currentVersion) {
                    throw VersionException(
                        formatString(
                            "The requested scene is from an older version of "
                            "Mitsuba "
                            "(file version: %s, current version: %s), hence "
                            "the loading process was stopped. "
                            "Please open the scene from within Mitsuba's "
                            "graphical user interface (mtsgui) -- "
                    ...
                    XMLLog(
                        EError,
                        "The requested scene is from an incompatible future "
                        "version of Mitsuba "
...
void SceneHandler::endElement(const XMLCh *const xmlName) {
    ...
    switch (tag.first) {
...
#ifdef WIN32
            int64_t i =
                _strtoi64(context.attributes["value"].c_str(), &end_ptr, 10);
#else
            int64_t i =
                strtoll(context.attributes["value"].c_str(), &end_ptr, 10);
#endif
            if (*end_ptr != '\0')
        ...
        case EFloat: {
            ...
            context.parent->properties.setFloat(context.attributes["name"],
                                                value);
        } break;
        ...
        case EBoolean: {
            ...
            if (str == "true") {
                ...
                XMLLog(EError,
                       "Unsupported boolean constant '%s' -- must be "
                       "'true' or 'false'!",
                       str.c_str());
                return;
            }
        ...
        case ERotate: {
            ...
            m_transform =
                Transform::rotate(Vector(x, y, z), angle) * m_transform;
        } break;

        case ELookAt: {
            std::vector<std::string> tokens =
                tokenize(context.attributes["origin"], ", ");
            if (tokens.size() != 3)
                XMLLog(EError, "<lookat>: invalid 'origin' argument");
            Point o(parseFloat(name, tokens[0]), parseFloat(name, tokens[1]),
                    parseFloat(name, tokens[2]));
            tokens = tokenize(context.attributes["target"], ", ");
            if (tokens.size() != 3)
                XMLLog(EError, "<lookat>: invalid 'target' argument");
            Point t(parseFloat(name, tokens[0]), parseFloat(name, tokens[1]),
                    parseFloat(name, tokens[2]));
            Vector u(0.0f);
            tokens = tokenize(context.attributes["up"], ", ");
            if (tokens.size() == 3)
                u = Vector(parseFloat(name, tokens[0]),
        ...
        case EScale: {
            bool hasXYZ = context.attributes["x"] != "" ||
                          context.attributes["y"] != "" ||
                          context.attributes["z"] != "";
            bool hasValue = context.attributes["value"] != "";
            Float x = 0, y = 0, z = 0;

            if (hasXYZ && hasValue) {
                XMLLog(EError,
                       "<scale>: provided both xyz and value arguments!");
            } else if (hasXYZ) {
                ...
                XMLLog(EError,
                       "<scale>: provided neither xyz nor value arguments!");
            }
        ...
        case EMatrix: {
            std::vector<std::string> tokens =
                tokenize(context.attributes["value"], ", ");
            if (tokens.size() != 16) XMLLog(EError, "Invalid matrix specified");
            int index = 0;
            Matrix4x4 mtx;
        ...
        case EPoint: {
            ...
            context.parent->properties.setPoint(context.attributes["name"],
                                                Point(x, y, z));
        } break;
        ...
        case EVector: {
            ...
            context.parent->properties.setVector(context.attributes["name"],
                                                 Vector(x, y, z));
        } break;

        case ERGB: {
            Spectrum::EConversionIntent intent = Spectrum::EReflectance;
            if (context.parent->tag == EEmitter) intent = Spectrum::EIlluminant;

            if (context.attributes.find("intent") != context.attributes.end()) {
                std::string intentString =
                    boost::to_lower_copy(context.attributes["intent"]);
                if (intentString == "reflectance")
                    ...
                    XMLLog(EError,
                           "Invalid intent \"%s\", must be "
                           "\"reflectance\" or \"illuminant\"",
                           intentString.c_str());
            }
        ...
        case ESRGB: {
            ...
            if (tokens.size() == 1 && tokens[0].length() == 7 &&
                ...
                int encoded = strtol(tokens[0].c_str() + 1, &end_ptr, 16);
                if (*end_ptr != '\0')
                    XMLLog(EError, "Invalid sRGB value specified (in <%s>)",
                           context.attributes["name"].c_str());
                value[0] = ((encoded & 0xFF0000) >> 16) / 255.0f;
                value[1] = ((encoded & 0x00FF00) >> 8) / 255.0f;
                value[2] = (encoded & 0x0000FF) / 255.0f;
        ...
        case EBlackBody: {
            ...
            if (temperature.length() > 0 &&
                std::toupper(temperature[temperature.length() - 1]) == 'K')
        ...
        case ESpectrum: {
            bool hasValue =
                context.attributes.find("value") != context.attributes.end();
            bool hasFilename =
                context.attributes.find("filename") != context.attributes.end();
            bool hasIntent =
                context.attributes.find("intent") != context.attributes.end();

            if (hasValue == hasFilename) {
                XMLLog(
                    EError,
                    "<spectrum>: please provide one of 'value' or 'filename'");
            } else if (hasFilename) {
                if (hasIntent)
                    XMLLog(EError,
                           "<spectrum>: 'intent' and 'filename' cannot be "
                           "specified at the same time!");
                FileResolver *resolver = Thread::getThread()->getFileResolver();
                fs::path path =
                    resolver->resolve(context.attributes["filename"]);
                InterpolatedSpectrum interp(path);
                interp.zeroExtend();
                Spectrum discrete;
                discrete.fromContinuousSpectrum(interp);
                discrete.clampNegative();
                context.parent->properties.setSpectrum(
                    context.attributes["name"], discrete);
            } else if (hasValue) {
                std::vector<std::string> tokens =
                    tokenize(context.attributes["value"], ", ");
                ...
                if (tokens.size() == 1 &&
                    ...
                    context.parent->properties.setSpectrum(
                        context.attributes["name"], spec);
                } else {
                    if (hasIntent)
                        XMLLog(EError,
                               "<spectrum>: 'intent' can only be specified "
                               "when given a single-valued argument.");
                    if (tokens[0].find(':') != std::string::npos) {
                        ...
                        for (size_t i = 0; i < tokens.size(); i++) {
                            ...
                            if (tokens2.size() != 2)
                                XMLLog(EError,
                                       "Invalid spectrum->value mapping "
                                       "specified");
                        ...
                        if (tokens.size() != SPECTRUM_SAMPLES)
                            XMLLog(EError,
                                   "Invalid spectrum value specified (length "
                                   "does not match the current spectral "
                                   "discretization!)");
                        for (int i = 0; i < SPECTRUM_SAMPLES; i++)
                            value[i] = parseFloat(name, tokens[i]);
                        context.parent->properties.setSpectrum(
                            context.attributes["name"], Spectrum(value));
                    }
                }
            }
        } break;
        ...
        case EInclude: {
            ...
            FileResolver *resolver = Thread::getThread()->getFileResolver();
            fs::path schemaPath =
                resolver->resolveAbsolute("data/schema/scene.xsd");
            ...
            SceneHandler *handler =
                new SceneHandler(m_params, m_namedObjects, true);
        ...
        case EDefault: {
            if (m_params.find(context.attributes["name"]) == m_params.end())
                m_params[context.attributes["name"]] =
                    context.attributes["value"];
        } break;
        ...
        default: {
            if (tag.second == NULL)
                XMLLog(EError,
                       "Internal error: could not instantiate an object "
                       "corresponding to the tag '%s'",
                       name.c_str());

            Properties &props = context.properties;

            /* Convenience hack: allow passing animated transforms to arbitrary
               shapes and then internally rewrite this into a shape group +
               animated instance */
            if (tag.second == MTS_CLASS(Shape) &&
                props.hasProperty("toWorld") &&
                props.getType("toWorld") == Properties::EAnimatedTransform &&
                (props.getPluginName() != "instance" &&
                 props.getPluginName() != "disk")) {
                /* (The 'disk' plugin also directly supports animated
                   transformations, so the instancing trick isn't required for
                   it) */

                ref<const AnimatedTransform> trafo =
                    props.getAnimatedTransform("toWorld");
                ...
                if (!trafo->isStatic()) {
                    ...
                    for (std::vector<std::pair<
                             std::string, ConfigurableObject *> >::iterator it =
                             context.children.begin();
                    ...
                    ref<Shape> shapeGroup =
                        static_cast<Shape *>(m_pluginManager->createObject(
                            MTS_CLASS(Shape), Properties("shapegroup")));
                ...
                try {
                    ...
                    XMLLog(EError, "Error while creating object: %s",
                           ex.what());
                }
            }
        } break;
        ...
        if (object) {
            ...
            if (context.parent != NULL) {
                ...
                context.parent->children.push_back(
                    std::pair<std::string, ConfigurableObject *>(nodeName,
                                                                 object));
            }
            ...
            if (name != "include" &&
                (!m_isIncludedFile ||
                 !object->getClass()->derivesFrom(MTS_CLASS(Scene))))
                object->configure();
    ...
    for (unsigned int i = 0; i < unq.size(); ++i)
        XMLLog(EWarn, "Unqueried attribute \"%s\" in element \"%s\"",
               unq[i].c_str(), name.c_str());
...
ref<Scene> SceneHandler::loadSceneFromString(const std::string &content,
                                             const ParameterMap &params) {
    /* Prepare for parsing scene descriptions */
    FileResolver *resolver = Thread::getThread()->getFileResolver();
    SAXParser *parser = new SAXParser();
    ...
    MemBufInputSource input((const XMLByte *)content.c_str(), content.length(),
                            inputName);
    parser->parse(input);
    ref<Scene> scene = handler->getScene();
...
void SceneHandler::staticShutdown() { XMLPlatformUtils::Terminate(); }
