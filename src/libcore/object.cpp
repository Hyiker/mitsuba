...
class RefCountTracker {
   ...
   public:
    ...
    ~RefCountTracker() {
        Lock lock(m_mutex);
        if (m_objects.size() == 0) {
            cout << "Reference count debugger -- all is good! (no leaked "
                    "objects)"
                 << endl;
        } else {
            cout << endl;
            cout << "Reference count debugger: leaked " << m_objects.size()
                 << " objects!" << endl;
            ...
            for (std::set<const Object *>::iterator it = m_objects.begin();
                ...
                cout
                    << "======================================================="
                    << endl;
                cout << " Leaked object at address " << (*it) << ":" << endl
                     << endl;
...
#if DEBUG_REFCOUNTS == 1
    if (__ref_tracker) __ref_tracker->add(this);
#endif
}
...
void Object::incRef() const {
    ...
    if (Class::rttiIsInitialized())
        cout << this << ": Increasing reference count ("
             << getClass()->getName() << ") -> " << (int)(m_refCount + 1)
             << endl;
#endif
#if defined(_MSC_VER)
...
void Object::decRef(bool autoDeallocate) const {
    ...
    if (count == 0 && autoDeallocate) {
        ...
        if (__ref_tracker) __ref_tracker->remove(this);
#endif
        delete this;
    }
}

const Class *Object::getClass() const { return m_theClass; }
...
Object::~Object() {
    if (m_refCount > 0) {
        Log(EWarn, "Deleting %s with reference count %i!", toString().c_str(),
            (int)m_refCount);
    }
}
...
#if DEBUG_REFCOUNTS == 1
    if (!__ref_tracker) __ref_tracker = new RefCountTracker();
#endif
}
