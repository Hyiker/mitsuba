...
struct MemoryMappedFile::MemoryMappedFilePrivate {
...
#if defined(__LINUX__) || defined(__OSX__)
        int fd =
            open(filename.string().c_str(), O_RDWR | O_CREAT | O_TRUNC, 0664);
        if (fd == -1)
            Log(EError, "Could not open \"%s\"!", filename.string().c_str());
        int result = lseek(fd, size - 1, SEEK_SET);
        if (result == -1)
            Log(EError, "Could not set file size of \"%s\"!",
                filename.string().c_str());
        result = write(fd, "", 1);
        if (result != 1)
            Log(EError, "Could not write to \"%s\"!",
                filename.string().c_str());
        data = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        if (data == NULL)
            Log(EError, "Could not map \"%s\" to memory!",
                filename.string().c_str());
    ...
    void createTemp() {
        ...
        int fd = mkstemp(path);
        if (fd == -1)
            SLog(EError, "Unable to create temporary file (1): %s",
                 strerror(errno));
        filename = path;
        free(path);
        ...
        if (ret == 0)
            SLog(EError, "GetTempFileName failed(): %s",
                 lastErrorText().c_str());

        file = CreateFileW(tempFilename, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                           CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    void map() {
        if (!fs::exists(filename))
            Log(EError, "The file \"%s\" does not exist!",
                filename.string().c_str());
        ...
        if (fd == -1)
            Log(EError, "Could not open \"%s\"!", filename.string().c_str());
        data = mmap(NULL, size, PROT_READ | (readOnly ? 0 : PROT_WRITE),
                    MAP_SHARED, fd, 0);
        if (data == NULL)
            Log(EError, "Could not map \"%s\" to memory!",
                filename.string().c_str());
        ...
        fileMapping = CreateFileMapping(
            file, NULL, readOnly ? PAGE_READONLY : PAGE_READWRITE, 0, 0, NULL);
        if (fileMapping == NULL)
            Log(EError, "CreateFileMapping: Could not map \"%s\" to memory: %s",
                filename.string().c_str(), lastErrorText().c_str());
        data = (void *)MapViewOfFile(
            fileMapping, readOnly ? FILE_MAP_READ : FILE_MAP_WRITE, 0, 0, 0);
        if (data == NULL)
            Log(EError, "MapViewOfFile: Could not map \"%s\" to memory: %s",
                filename.string().c_str(), lastErrorText().c_str());
#endif
    ...
    void unmap() {
        SLog(ETrace, "Unmapping \"%s\" from memory", filename.string().c_str());
        ...
        if (temp) {
            ...
            if (retval != 0)
                Log(EError, "munmap(): unable to unmap memory: %s",
                    strerror(errno));
        }
        ...
        if (!UnmapViewOfFile(data))
            Log(EError, "UnmapViewOfFile(): unable to unmap memory: %s",
                lastErrorText().c_str());
        if (!CloseHandle(fileMapping))
            Log(EError, "CloseHandle(): unable to close file mapping: %s",
                lastErrorText().c_str());
        if (!CloseHandle(file))
            Log(EError, "CloseHandle(): unable to close file: %s",
                lastErrorText().c_str());
#endif
        ...
        if (temp) {
            try {
                ...
                Log(EWarn, "unmap(): Unable to delete file \"%s\"",
                    filename.string().c_str());
            }
        }
...
MemoryMappedFile::MemoryMappedFile() : d(new MemoryMappedFilePrivate()) {}
...
void MemoryMappedFile::resize(size_t size) {
    if (!d->data) Log(EError, "Internal error in MemoryMappedFile::resize()!");
    bool temp = d->temp;
    d->temp = false;
    d->unmap();
...
void *MemoryMappedFile::getData() { return d->data; }

/// Return a pointer to the file contents in memory (const version)
const void *MemoryMappedFile::getData() const { return d->data; }

size_t MemoryMappedFile::getSize() const { return d->size; }

bool MemoryMappedFile::isReadOnly() const { return d->readOnly; }

const fs::path &MemoryMappedFile::getFilename() const { return d->filename; }
...
std::string MemoryMappedFile::toString() const {
    ...
    oss << "MemoryMappedFile[filename=\"" << d->filename.string()
        << "\", size=" << memString(d->size) << "]";
    return oss.str();
}
