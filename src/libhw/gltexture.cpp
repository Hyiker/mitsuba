...
void GLTexture::init() {
    ...
    if (m_samples > 1) {
        ...
        if (m_samples > maxSamples) {
            Log(EWarn,
                "Attempted to create a multisample framebuffer "
                "with an unsupported number of samples (requested=%i, "
                "supported=%i)",
    ...
    if (m_fbType == ENone) {
        ...
        switch (m_fbType) {
                ...
                if (m_type == ETexture2D) {
                    if (!depthAsTexture) {
                        ...
                        glTexParameteri(m_glType, GL_TEXTURE_COMPARE_MODE,
                                        GL_NONE);
                        glTexParameteri(m_glType, GL_DEPTH_TEXTURE_MODE,
                                        GL_LUMINANCE);
                        if (m_samples == 1)
                            glTexImage2D(m_glType, 0, GL_DEPTH_COMPONENT32,
                                         m_size.x, m_size.y, 0,
                                         GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE,
                                         NULL);
                        else
                            glTexImage2DMultisample(
                                m_glType, m_samples, GL_DEPTH_COMPONENT32,
                                m_size.x, m_size.y, GL_FALSE);
                        glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
                                                  GL_DEPTH_ATTACHMENT, m_glType,
                                                  m_depthId, 0);
                        glBindTexture(m_glType, m_id);
                    }

                    if (m_samples == 1)
                        glTexImage2D(m_glType, 0, m_internalFormat, m_size.x,
                                     m_size.y, 0, m_format, m_dataFormat, NULL);
                    else
                        glTexImage2DMultisample(m_glType, m_samples,
                                                m_internalFormat, m_size.x,
                                                m_size.y, GL_FALSE);
                    ...
                    Assert(m_size.x == m_size.y &&
                           math::isPowerOfTwo(m_size.x));
                    Assert(m_fbType == EColorBuffer);
                    Assert(m_samples == 1);

                    for (int i = 0; i < 6; i++)
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
                                     m_internalFormat, m_size.x, m_size.y, 0,
                                     m_format, m_dataFormat, NULL);
                    ...
                    if (depthAsTexture) {
                        ...
                        glGenTextures(1, &m_depthId);
                        glBindTexture(m_glType, m_depthId);
                        glTexParameteri(m_glType, GL_TEXTURE_MAG_FILTER,
                                        GL_LINEAR);
                        glTexParameteri(m_glType, GL_TEXTURE_MIN_FILTER,
                                        GL_LINEAR);

                        for (int i = 0; i < 6; i++)
                            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
                                         GL_DEPTH_COMPONENT32, m_size.x,
                                         m_size.y, 0, GL_DEPTH_COMPONENT,
            ...
            case EDepthBuffer:
                ...
                if (m_depthMode == ECompare) {
                    glTexParameteri(m_glType, GL_TEXTURE_COMPARE_FUNC,
                                    GL_LEQUAL);
                    glTexParameteri(m_glType, GL_TEXTURE_COMPARE_MODE,
                ...
                if (m_type == ETexture2D) {
                    ...
                    glTexImage2D(m_glType, 0, m_internalFormat, m_size.x,
                                 m_size.y, 0, GL_DEPTH_COMPONENT, m_dataFormat,
                                 NULL);

                    /* Attach the texture as a depth target */
                    glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
                                              GL_DEPTH_ATTACHMENT_EXT, m_glType,
                                              m_id, 0);
                } else if (m_type == ETextureCubeMap) {
                    Assert(m_size.x == m_size.y &&
                           math::isPowerOfTwo(m_size.x));
        ...
        switch (errorStatusID) {
            ...
            case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_EXT:
                errorStatus = "Incomplete attachment";
                break;
            case GL_FRAMEBUFFER_UNSUPPORTED_EXT:
                errorStatus = "Unsupported framebuffer format";
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_DUPLICATE_ATTACHMENT_EXT:
                errorStatus = "Incomplete framebuffer - duplicate attachment";
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_EXT:
                errorStatus = "Incomplete framebuffer - missing attachment";
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
                errorStatus = "Incomplete framebuffer - invalid dimensions";
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT:
                errorStatus = "Incomplete framebuffer - no draw buffer";
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
                errorStatus = "Incomplete framebuffer - invalid formats";
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER_EXT:
                errorStatus = "Incomplete framebuffer - no readbuffer";
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE_EXT:
                errorStatus = "Incomplete multisample framebuffer";
                break;
            case GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS:
                errorStatus = "Incomplete layer targets";
                break;
            default:
                errorStatus = "Unknown error status";
                break;
        }
...
void GLTexture::refresh(const Point2i &offset, const Vector2i &size) {
    ...
    glTexSubImage2D(m_glType, 0, offset.x, offset.y, size.x, size.y, m_format,
                    m_dataFormat, ptr);
...
void GLTexture::refresh() {
    ...
    if (m_type == ETexture1D) {
        ...
        if (isMipMapped()) {
            ...
            // FIXME
            // gluBuild1DMipmaps(m_glType, m_internalFormat,
            //                   m_size.x == 1 ? m_size.y : m_size.x, m_format,
            //                   m_dataFormat, bitmap->getData());
        } else {
        }
        glTexImage1D(m_glType, 0, m_internalFormat,
                     m_size.x == 1 ? m_size.y : m_size.x, 0, m_format,
                     m_dataFormat, bitmap->getData());
        ...
        glTexImage2D(m_glType, 0, m_internalFormat, m_size.x, m_size.y, 0,
        ...
        for (int i = 0; i < 6; i++) {
            ...
            GLuint pos;
            switch (i) {
                case ECubeMapPositiveX:
                    pos = GL_TEXTURE_CUBE_MAP_POSITIVE_X;
                    break;
                case ECubeMapNegativeX:
                    pos = GL_TEXTURE_CUBE_MAP_NEGATIVE_X;
                    break;
                case ECubeMapPositiveY:
                    pos = GL_TEXTURE_CUBE_MAP_POSITIVE_Y;
                    break;
                case ECubeMapNegativeY:
                    pos = GL_TEXTURE_CUBE_MAP_NEGATIVE_Y;
                    break;
                case ECubeMapPositiveZ:
                    pos = GL_TEXTURE_CUBE_MAP_POSITIVE_Z;
                    break;
                case ECubeMapNegativeZ:
                    pos = GL_TEXTURE_CUBE_MAP_NEGATIVE_Z;
                    break;
                default:
                    Log(EError, "Unknown cube map index");
                    return;
            };

            glTexImage2D(pos, 0, m_internalFormat, bitmap->getWidth(),
                         bitmap->getHeight(), 0, m_format, m_dataFormat,
                         bitmap->getData());
        }
    } else {
...
void GLTexture::lookupGLConstants() {
    ...
    switch (m_type) {
        ...
        case ETextureCubeMap:
            m_glType = GL_TEXTURE_CUBE_MAP_EXT;
            break;
        default:
            Log(EError, "Invalid texture type specified");
            return;
    }
...
void GLTexture::configureTexture() {
    ...
    switch (m_wrapTypeU) {
        ...
        case ERepeat:
            ...
            break;
        case EMirror:
            wrapU = GL_MIRRORED_REPEAT_ARB;
            break;
        default:
            Log(EError, "Invalid texture wrap type specified");
            return;
    }
    ...
    switch (m_wrapTypeV) {
        ...
        case ERepeat:
            ...
            break;
        case EMirror:
            wrapV = GL_MIRRORED_REPEAT_ARB;
            break;
        default:
            Log(EError, "Invalid V texture wrap type specified");
            return;
    }
    ...
    if (border && m_type == ETexture2D) {
        const GLfloat color[] = {(GLfloat)m_borderColor[0],
                                 (GLfloat)m_borderColor[1],
                                 (GLfloat)m_borderColor[2], (GLfloat)1.0f};
...
void GLTexture::download(Bitmap *bitmap) {
    ...
    switch (bitmap->getComponentFormat()) {
        ...
        case Bitmap::EFloat16:
            dataFormat = GL_HALF_FLOAT_ARB;
            break;
        case Bitmap::EFloat32:
            dataFormat = GL_FLOAT;
            break;
        case Bitmap::EFloat64:
            dataFormat = GL_DOUBLE;
            break;
        default:
            Log(EError,
                "GLTexture::download(): Unknown/unsupported component format "
                "%i!",
    ...
    switch (bitmap->getPixelFormat()) {
        ...
        default:
            Log(EError,
                "GLTexture::download(): Unknown/unsupported pixel format %i!",
        ...
        case ETextureCubeMap:
            for (int i = 0; i < 6; ++i) {
                ...
                glReadPixels(0, 0, bitmap->getWidth(), bitmap->getHeight(),
                             format, dataFormat, bitmap->getUInt8Data());
                bitmap->flipVertically();
            }
            break;
        default:
...
void GLTexture::activateSide(int side) {
    if (side == -1) {
        if (m_fbType == EColorBuffer) {
            Log(EError,
                "GLTexture::activateTexture(-1): Not allowed for cube map "
                "color-only buffers");
        } else if (m_fbType == EColorAndDepthBuffer) {
            glFramebufferTextureEXT(GL_FRAMEBUFFER_EXT,
                                    GL_COLOR_ATTACHMENT0_EXT, m_id, 0);
            glFramebufferTextureEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT,
                                    m_depthId, 0);
...
void GLTexture::releaseTarget() {
    ...
    glPopAttrib();
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, GL_NONE);
    if (isMipMapped()) m_needsUpdate = true;
}
...
void GLTexture::bind(int textureUnit, int textureIndex) const {
    ...
    if (GLEW_VERSION_1_3) {
        ...
        if (textureUnit != 0) Log(EError, "Multitexturing is not supported");
    }
...
void GLTexture::blit(GPUTexture *target, int what) const {
    ...
    if (!GLEW_EXT_framebuffer_blit)
        Log(EError,
            "Your OpenGL driver does not support fast "
    ...
    int flags = 0;
    if (what & EDepthBuffer) flags |= GL_DEPTH_BUFFER_BIT;
    if (what & EColorBuffer) flags |= GL_COLOR_BUFFER_BIT;

    if (!dest) {
        glBlitFramebufferEXT(0, 0, m_size.x, m_size.y, 0, 0, m_size.x, m_size.y,
                             flags, GL_NEAREST);
    } else {
        glBlitFramebufferEXT(0, 0, m_size.x, m_size.y, 0, 0, dest->m_size.x,
                             dest->m_size.y, flags,
...
void GLTexture::blit(GPUTexture *target, int what, const Point2i &sourceOffset,
    ...
    glBlitFramebufferEXT(
        sourceOffset.x, sourceOffset.y, sourceOffset.x + sourceSize.x,
        sourceOffset.x + sourceSize.y, destOffset.x, destOffset.y,
        destOffset.x + destSize.x, destOffset.y + destSize.y, flags,
