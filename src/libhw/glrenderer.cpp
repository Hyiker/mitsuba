...
// GLEWContextStruct *glewGetContext() { return &glewContext.get(); }
...
FINLINE void loadMatrix(const Matrix4x4 &mat) {
    ...
    for (int j = 0; j < 4; j++)
        for (int i = 0; i < 4; i++) temp[pos++] = (GLfloat)mat(i, j);
...
FINLINE Matrix4x4 fetchMatrix(GLenum which) {
    ...
    for (int j = 0; j < 4; j++)
        for (int i = 0; i < 4; i++) mat(i, j) = (Float)temp[pos++];
...
GLRenderer::GLRenderer(Session *session) : Renderer(session) {}
...
void GLRenderer::init(Device *device, Renderer *other) {
    ...
    if (glewIsSupported("GL_EXT_framebuffer_object")) {
        m_capabilities->setSupported(RendererCapabilities::ERenderToTexture,
                                     true);
        Log(m_logLevel, "Capabilities: Framebuffers objects are supported.");
    } else {
        Log(m_warnLogLevel,
            "Capabilities: Framebuffers objects are NOT supported!");
    }

    if (glewIsSupported("GL_ARB_shading_language_100")) {
        m_capabilities->setSupported(RendererCapabilities::EShadingLanguage,
                                     true);
    ...
    if (glewIsSupported("GL_ARB_texture_float")) {
        ...
        Log(m_warnLogLevel,
            "Capabilities: Floating point textures are NOT supported!");
    }

    if (glewIsSupported("GL_ARB_color_buffer_float")) {
        m_capabilities->setSupported(RendererCapabilities::EFloatingPointBuffer,
                                     true);
        Log(m_logLevel,
            "Capabilities: Floating point color buffers are supported.");
    } else {
        Log(m_warnLogLevel,
            "Capabilities: Floating point color buffers are NOT supported!");
    }

    if (glewIsSupported("GL_EXT_framebuffer_blit")) {
        m_capabilities->setSupported(RendererCapabilities::EBufferBlit, true);
        Log(m_logLevel, "Capabilities: Fast buffer blitting is supported.");
    } else {
        Log(m_warnLogLevel,
            "Capabilities: Fast buffer blitting is NOT supported!");
    }
    ...
    if (glewIsSupported("GL_EXT_framebuffer_multisample") &&
        ...
        Log(m_logLevel,
            "Capabilities: Multisample framebuffer objects are supported.");
    } else {
    ...
    if (glewIsSupported("GL_ARB_vertex_buffer_object")) {
        m_capabilities->setSupported(RendererCapabilities::EVertexBufferObjects,
                                     true);
        Log(m_logLevel, "Capabilities: Vertex buffer objects are supported.");
    } else {
        Log(m_warnLogLevel,
            "Capabilities: Vertex buffer objects are NOT supported!");
    }

    if (glewIsSupported("GL_EXT_geometry_shader4") &&
        glewIsSupported("GL_EXT_gpu_shader4")) {
        m_capabilities->setSupported(RendererCapabilities::EGeometryShaders,
                                     true);
        Log(m_logLevel, "Capabilities: Geometry shaders are supported.");
    } else {
        Log(m_warnLogLevel,
            "Capabilities: Geometry shaders are NOT supported!");
    }

    if (glewIsSupported("GL_ARB_shader_texture_lod") ||
        glewIsSupported("GL_EXT_gpu_shader4")) {
        m_capabilities->setSupported(
        ...
        Log(m_warnLogLevel,
            "Capabilities: Custom texture filtering is NOT supported.");
    }
    ...
    radeonOnOSX = boost::to_lower_copy(m_driverRenderer).find("radeon") !=
                  std::string::npos;
#endif

    if (glewIsSupported("GL_ARB_sync") && !radeonOnOSX) {
        m_capabilities->setSupported(RendererCapabilities::ESyncObjects, true);
        Log(m_logLevel, "Capabilities: Synchronization objects are supported.");
    } else {
        Log(m_warnLogLevel,
            "Capabilities: Synchronization objects are NOT supported!");
    }

    if (glewIsSupported("GL_NV_vertex_buffer_unified_memory")) {
        m_capabilities->setSupported(RendererCapabilities::EBindless, true);
...
void GLRenderer::shutdown() { Renderer::shutdown(); }
...
GPUSync *GLRenderer::createGPUSync() { return new GLSync(); }
...
void GLRenderer::checkError(bool onlyWarn) {
    ...
    // FIXME
    // if (glError)
    //     Log(onlyWarn ? m_warnLogLevel : EError, "OpenGL Error : %s",
    ...
    if (it != m_geometry.end()) {
        ...
        if (EXPECT_TAKEN(m_queuedTriangles + size < MTS_GL_MAX_QUEUED_TRIS)) {
            ...
            glDrawElements(GL_TRIANGLES,
                           (GLsizei)(mesh->getTriangleCount() * 3),
            ...
            while (cur < size) {
                size_t drawAmt = std::min(
                    size - cur, MTS_GL_MAX_QUEUED_TRIS - m_queuedTriangles);
                if (drawAmt > 0)
                    glDrawElements(GL_TRIANGLES, (GLsizei)(drawAmt * 3),
                                   GL_UNSIGNED_INT, indices + cur * 3);
                m_queuedTriangles += drawAmt;
                cur += drawAmt;
                if (cur < size) finish();
            }
        }
...
void GLRenderer::drawMesh(const GPUGeometry *_geo) {
    ...
    if (m_capabilities->isSupported(RendererCapabilities::EBindless)) {
        ...
        glBufferAddressRangeNV(GL_VERTEX_ARRAY_ADDRESS_NV, 0, vertexAddr,
                               vertexSize);
        ...
        glBufferAddressRangeNV(GL_ELEMENT_ARRAY_ADDRESS_NV, 0, indexAddr,
                               indexSize);
    } else {
    ...
    if (EXPECT_TAKEN(m_queuedTriangles + size < MTS_GL_MAX_QUEUED_TRIS)) {
        /* Draw all triangles */
        glDrawElements(GL_TRIANGLES, (GLsizei)(size * 3), GL_UNSIGNED_INT,
                       (GLvoid *)0);
        m_queuedTriangles += size;
    } else {
        /* Spoon-feed them (keeps the OS responsive) */
...
void GLRenderer::drawAll(
    ...
    if (m_capabilities->isSupported(RendererCapabilities::EBindless)) {
        for (std::vector<TransformedGPUGeometry>::const_iterator it =
                 allGeometry.begin();
             it != allGeometry.end(); ++it) {
            const GLGeometry *geo =
                static_cast<const GLGeometry *>((*it).first);
            ...
            if (EXPECT_TAKEN(m_queuedTriangles + size <
                             MTS_GL_MAX_QUEUED_TRIS)) {
                /* Draw all triangles */
    ...
    if (!curObjTrafo.isIdentity()) loadMatrix(backup);
}

void GLRenderer::blitTexture(const GPUTexture *tex, bool flipVertically,
                             bool centerHoriz, bool centerVert,
    ...
    if (centerHoriz) upperLeft.x = (scrSize.x - texSize.x) / 2;
    if (centerVert) upperLeft.y = (scrSize.y - texSize.y) / 2;
    upperLeft += offset;
    lowerRight = upperLeft + texSize;

    if (flipVertically) std::swap(upperLeft.y, lowerRight.y);
...
void GLRenderer::drawText(const Point2i &_pos, const Font *font,
    ...
    for (size_t i = 0; i < text.length(); i++) {
        ...
        Point2 start = Point2(pos + Vector2i(glyph.horizontalBearing,
                                             font->getMaxVerticalBearing() -
                                                 glyph.verticalBearing));
...
void GLRenderer::setPointSize(Float size) { glPointSize((GLfloat)size); }
...
void GLRenderer::drawEllipse(const Point &center, const Vector &axis1,
                             const Vector &axis2) {
...
#define V(a, b, c) \
    glVertex3f((GLfloat)aabb.a.x, (GLfloat)aabb.b.y, (GLfloat)aabb.c.z)
    glBegin(GL_LINE_LOOP);
    V(max, min, max);
    V(max, min, min);
    V(max, max, min);
...
Matrix4x4 GLRenderer::getMatrix(EMatrixType type) const {
    return fetchMatrix(type == EProjection ? GL_PROJECTION_MATRIX
                                           : GL_MODELVIEW_MATRIX);
}

void GLRenderer::setCamera(const ProjectiveCamera *camera,
                           const Point2 &apertureSample, const Point2 &aaSample,
                           Float timeSample) {
    Float time =
        camera->getShutterOpen() + camera->getShutterOpenTime() * timeSample;

    glMatrixMode(GL_PROJECTION);
    loadMatrix(
        camera->getProjectionTransform(apertureSample, aaSample).getMatrix());
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
...
void GLRenderer::setColor(const Color3 &col, Float alpha) {
    glColor4f((GLfloat)col[0], (GLfloat)col[1], (GLfloat)col[2],
              (GLfloat)alpha);
}
...
void GLRenderer::setClearDepth(Float depth) { glClearDepth((GLfloat)depth); }

void GLRenderer::setClearColor(const Color3 &color) {
    glClearColor((GLfloat)color[0], (GLfloat)color[1], (GLfloat)color[2], 1.0f);
}
...
void GLRenderer::debugString(const std::string &text) {
    if (GLEW_GREMEDY_string_marker) glStringMarkerGREMEDY(0, text.c_str());
}
